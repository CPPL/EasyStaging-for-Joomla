        ...
        if ($status) {
            ...
            if (($rsyncResult != false) && ($rsyncResult == 0)) {
                ...
                if ($rsyncAction == self::RSYNC_CLEAR) {
                    $this->log(
                        $theStep,
                        JText::sprintf('COM_EASYSTAGING_CLI_RSYNC_CLEAN_X', $details->label, $rsyncCmd)
                    );
...
DEF;
    ...
    private function doMergeRecords($step, $tableProfiles)
    {
        ...
        // Yes I hate magic numbers too but this works for the majority of MySQL hosts we've encountered
        // @todo Magic Number run around, scream and panic
    ...
    private function createCopyTableExportFile($step)
    {
        ...
        // 3. Next we try and get the records in the table (after all no point
        // in creating an insert statement if there are no records :D )
        ...
        if (($records = $this->source_db->loadRowList()) != null) {
            ...
            if ($flds) {
                ...
                foreach ($records as $row) {
                    ...
                    // First up we check to see if this row will put our SQL block size
                    // over our max_packet value on the target server
                    $rowSize = strlen($rowAsValues);

                    // Have we reached our block size? If so, add the current data to
                    // the build SQL and reset for next block of SQL.
                 ...
                 * (i.e. a column name that the current db doesn't like)
                 * Typical causes are columns names that start with a number
                 * or other illegal character or are completely numeric
                 *
                 */
