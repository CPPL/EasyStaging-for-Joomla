 ...
 * So, it turns out that on badly setup CentOS servers at hosts that aren't that cluey
 * this check will fail when a php-cli is setup for setups using fcgi
 **/
...
{
    die("Critical Joomla libraries couldn't be found.");
}
...
{
    die("Joomla configuration file couldn't be found.");
}
...
class EasyStaging_PlanRunner extends JApplicationCli
{
                    ...
                    if ($steps) {
                        ...
                        foreach ($steps as $step) {
                            ...
                            switch ($step['action_type']) {
                                ...
                                default:
                                    // Anything else we discard, who knows what crazyness caused this...
                                    // best to avoid potential damage by doing nothing!
                            }
    ...
    private function performRSYNC($theStep)
    {
        ...
        if ($rsResult['status']) {
            ...
            RunHelper::markCompleted(
                $theStep,
                JText::sprintf('COM_EASYSTAGING_JSON_RSYNC_STEP_FAILED', $this->plan_id)
            );
        }
    ...
    protected function runRsync($theStep, $filename)
    {
        ...
        if ($status) {
            ...
            $this->log($theStep, JText::sprintf('COM_EASYSTAGING_CLI_RSYNC_CMD_X_Y', $details->label, $rsyncCmd));
            ...
            if (($rsyncResult != false) && ($rsyncResult == 0)) {
                // It ended cleanly.
                $status = true;
                $this->log($theStep, JText::_('COM_EASYSTAGING_CLI_RSYNC_INDICATES_SUCCESS'));

                // Is it a MOVE file action?
                if ($rsyncAction == self::RSYNC_CLEAR) {
                    $this->log($theStep, JText::sprintf(
                        'COM_EASYSTAGING_CLI_RSYNC_CLEAN_X',
                        $details->label,
                        $rsyncCmd
                    ));

                    // We make an empty directory
                    $emptyDir = $this->runFilesPath() . '/empty';
                ...
                $this->log($theStep, JText::_('COM_EASYSTAGING_CLI_RSYNC_INDICATES_FAILURE'));
            }
        }
    ...
    private function runRsyncCmd($rsyncCmd, $theStep)
    {
        ...
        if (is_resource($rsync_process)) {
            ...
            $this->log($theStep, JText::_('COM_EASYSTAGING_CLI_RSYNC_PROC_OPEN_FAILED'));
        }
     ...
     * @return array
     */
    private function createRSYNCExclusionFile($theStep)
    {
        $this->log($theStep, JText::_('COM_EASYSTAGING_CLI_START_RSYNC_STEP1'));
        $decoded_details = json_decode($theStep->action);

        // Build our file path & file handle
        $pathToExclusionsFile = $this->getRunDirectory() . '/' . $this->exclFileName($decoded_details->id);
        $result = array(
            'fileName' => $pathToExclusionsFile,
        );
        $result['fullPathToExclusionFile'] = $this->syncFilesPath() . $pathToExclusionsFile;
...
DEF;
            ...
            $allExclusions = $defaultExclusions . trim($this->checkExclusionField($decoded_details->file_exclusions));
            $result['fileData'] = $allExclusions;

            $this->log($theStep, $allExclusions . "\n");

            // Attempt to write the file
            $result['status'] = fwrite($exclusionFile, $allExclusions);
            $result['msg'] = $result['status'] ? JText::sprintf(
                'COM_EASYSTAGING_FILE_WRITTEN_SUCCESSFULL_DESC',
                $result['status']
            )
     ...
     */
    private function checkExclusionField($file_exclusions)
    {
    ...
    private function performTableStep($step)
    {
        ...
        $this->log($step, JText::sprintf('COM_EASYSTAGING_CLI_STARTING_TABLE_X', $step->action));
        ...
        if ($this->db_status) {
            ...
            if ($status) {
                $msg = JText::sprintf(
                    'COM_EASYSTAGING_CLI_RESULT_FOR_TABLE_X_' . $actiontype . '_STEP_SUCCESS',
                    $step->action
                );
            } else {
                $msg = JText::sprintf(
                    'COM_EASYSTAGING_CLI_RESULT_FOR_TABLE_X_' . $actiontype . '_STEP_FAILURE',
                    $step->action
                );
            }
        } else {
    ...
    private function performTableCopy($step)
    {
        ...
        if ($this->createCopyTableExportFile($step)) {
            // Run the table copy
    ...
    private function doMergeRecords($step, $tableProfiles)
    {
        ...
        // Yes I hate magic numbers too but this works for the majority of
        // MySQL hosts we've encountered @todo Magic Number run around, scream and panic
        ...
        if ($createTempResult = $trgDB->execute()) {
            ...
            for ($i = 1; $i <= $exportSteps; $i++) {
                ...
                if (!$insertResult = $this->insertRows($rows, $trgDB, $tempTable)) {
                    $mergeResult = false;
                    $this->log($step, JText::sprintf('COM_EASYSTAGING_CLI_MERGE_BACK_FAILED_FOR_X', $srcTableName));
                    break;
                } else {
                    $this->log(
                        $step,
            ...
            if ($mergeResult) {
                ...
                if (!$trgDB->execute()) {
                    $mergeResult = false;
                    $this->log($step, JText::sprintf('COM_EASYSTAGING_CLI_INSERT_IGNORE_FAILED_FOR_X', $srcTableName));
                }
            }
        }
    ...
    private function checkDBConnection($theStep)
    {
        ...
        // Get our DB objects, assuming for now that we're dealing with PUSH action,
        // so the source is local & target is remote
        $this->target_db = JDatabaseDriver::getInstance($options);
        $this->source_db  = JFactory::getDbo();

        try {
            $this->target_db->connect();
            ...
            if ($qr) {
                ...
                if ($this->targetTablesRetreived) {
                    ...
                    $this->log($theStep, $tableList);
                    $msg = JText::sprintf(
                        'COM_EASYSTAGING_DATABASE_STEP_01_FOUND_X_TABLES',
                        count($this->targetTablesRetreived),
            ...
            $msg = JText::sprintf('COM_EASYSTAGING_DATABASE_STEP_01_FAILED_TO_CONNECT_X', $e->getMessage());
            $this->log($theStep, $msg);
        }
     ...
     * @return  bool
     */
    private function createCopyTableExportFile($step)
    {
        ...
        // Build our file path & file handle
        $pathToSQLFile = $this->syncFilesPath() .
            $this->getRunDirectory() . '/' .
            $this->exportCopyFileName($table);
        ...
        $hasAFilter = $this->filterTable($table);
        ...
        // 3. Next we try and get the records in the table
        // (after all no point in creating an insert statement if there are no records :D )
        /** @var $dbq JDatabaseQuery */
        $dbq = $this->source_db->getQuery(true);

        // @todo really? all of the fields — lets fix this.
        $dbq->select('*');
        $dbq->from($sourceTableName);

        // If our table has an exclusion filter we need to add a 'where' element to our query.
        if ($hasAFilter) {
            $fieldToCompare = key($hasAFilter);
            $valueToAvoid = $hasAFilter[$fieldToCompare];
            $condition = $this->source_db->quoteName($fieldToCompare) .
                ' NOT LIKE ' .
                $this->source_db->quote($valueToAvoid);
            $dbq->where($condition);
        }
        ...
        if (($records = $this->source_db->loadRowList()) != null) {
            ...
            $flds = $this->getArrayOfFieldNames($table, $this->source_db);
            ...
            if ($flds) {
                ...
                foreach ($records as $row) {
                    ...
                    // First up we check to see if this row will put our SQL block
                    // size over our max_packet value on the target server
                    $rowSize = strlen($rowAsValues);

                    // Have we reached our block size? If so, add the current
                    // data to the build SQL and reset for next block of SQL.
                ...
                $endofSQL = $this->endOfExportSQL($table);
                ...
                if ($exportSQLFile = @fopen($pathToSQLFile, 'w')) {
                    ...
                    $msg .= JText::sprintf(
                        'COM_EASYSTAGING_CLI_PATH_TO_SQL_EXPORT_FILE_X_FOR_TABLE_Y',
                        $pathToSQLFile,
                        $table
                    );
                ...
                /**
                 * Ahh... bugger, Joomla! found a column name it didn't like
                 * (i.e. a column name that the current db doesn't like) Typical causes
                 * are columns names that start with a number or other illegal character or are completely numeric
                 *
                 */
     ...
     * @return  string
     */
    private function endOfExportSQL($table)
    {
    ...
    private function runTableCopyExport($step)
    {
        ...
        $pathToSQLFile = $this->syncFilesPath() .
            $this->getRunDirectory() . '/' .
            $this->exportCopyFileName($tableName);
        ...
        if (($pathToSQLFile != '') && (file_exists($pathToSQLFile))) {
            ...
            // Update progress
            $msg .= JText::sprintf(
                'COM_EASYSTAGING_CLI_EXPORT_FILE_X_HAS_Y_STATEMENTS',
                $tableName,
                count($exportSQLQuery)
            );
            $this->log($step, $msg);
            ...
            if (count($exportSQLQuery)) {
                ...
                if ($this->target_db) {
                    ...
                    foreach ($exportSQLQuery as $query) {
                        if (!empty($query)) {
                            ...
                            try {
                                $this->target_db->execute();

                                if (($first_word == 'SET' && $last_word == 'UNLOCK') ||
                                    ($first_word == 'SET' && $finishing)) {
                                    $first_word = 'UNSET';
                                    $finishing = true;
                                }
                                ...
                                if (($first_word == 'SET' && $last_word != 'SET')
                                    ...
                                    $msg = JText::sprintf(
                                        'COM_EASYSTAGING_CLI_TABLE_EXPORT_QUERY_' . strtoupper($first_word),
                                        $newTableName,
                                        $target_site->database_name
                                    );
                                }
                ...
                /**
                 * @todo Confirm result, how? Check a matching number of records?
                 * What else? Maybe check the create statement?.
                 */
            } else {
     ...
     */
    private function getArrayOfFieldNames($table, $db)
    {
    ...
    private function writeToLog($logLine)
    {
        ...
        if (!is_array($runDirectory) && $this->runticket) {
            if (!isset($this->logFile) || ($this->logFile === false)) {
                ...
                $fullPathToLogFile = JPATH_COMPONENT_ADMINISTRATOR . '/syncfiles/' .
                    $this->runticket . '/' . $logFileName;
     ...
     * @return string
     */
    private function syncFilesPath()
    {
        return JPATH_COMPONENT_ADMINISTRATOR . '/syncfiles/';
    }

    private function runFilesPath()
    {
        return JPATH_COMPONENT_ADMINISTRATOR . '/syncfiles/' . $this->getRunDirectory();
    }
     ...
     */
    private function getRunDirectory($runDirectory = null)
    {
        ...
        if ($runDirectory) {
            if (!file_exists($runDirectoryPath)) {
                if (mkdir($runDirectoryPath, 0777, true)) {
                    ...
                    $result['status'] = 0;
                    $result['msg'] = JText::sprintf(
                        'COM_EASYSTAGING_PLAN_JSON_UNABLE_TO_CREAT_RUN_DIR',
                        $runDirectoryPath
                    );
                }
            } else {
    ...
    /**
     * Central pont for defining the name of the exclusions file for the rsync call,
     * later this will use a preference setting.
     *
     * @param   int  $rsyncId  ID of the File Copy Action.
     *
     * @return string
     */
    private function exclFileName($rsyncId)
    {
        return ('plan-' . $this->planId() . '-' . $rsyncId . '-exclusions.txt');
    }
     ...
     * @return string
     */
    private function exportCopyFileName($table)
    {
        return ('plan-' . $this->planId() . '-' . $table . '-export.sql');
    }
     ...
     * @return string
     */
    private function rsyncOutputFileName()
    {
    ...
    private function finishRun($plan_id)
    {
        ...
        $this->log($this->rootStep, $exitMsg);
